...
#define ECHO_TONE     3
#define ECHO_BITS     4  // 1 to 24 bits
#define ECHO_SR_DIV   5  // Sample Rate reduction factor
...
void runEcho(LADSPA_Handle Instance, unsigned long SampleCount) {
    ...
    // Pre-calculate bit crushing levels
    ...
    for (unsigned long i = 0; i < SampleCount; i++) {
        ...
        // --- LO-FI PROCESSING (Internal only) ---
        ...
        loL = floorf(loL * levels) / levels;
        loR = floorf(loR * levels) / levels;
        ...
        // Feedback loop (Drive applied to input only)
        p->bufferL[p->writeIdx] = saturate(inL, drive) + (lpL * feedback);
        p->bufferR[p->writeIdx] = saturate(inR, drive) + (lpR * feedback);
        p->writeIdx = (p->writeIdx + 1) % MAX_DELAY;

        // Final Mix (inL/inR is untouched)
        p->ports[ECHO_OUT_L][i] = (inL * (1.0f - mix)) + (lpL * mix);
        p->ports[ECHO_OUT_R][i] = (inR * (1.0f - mix)) + (lpR * mix);
    }
}
...
const LADSPA_Descriptor * ladspa_descriptor(unsigned long Index) {
    ...
    if (!g_desc) {
        ...
        h[6].LowerBound=0; h[6].UpperBound=200; h[6].HintDescriptor=LADSPA_HINT_BOUNDED_BELOW|LADSPA_HINT_BOUNDED_ABOVE|LADSPA_HINT_DEFAULT_MINIMUM;
