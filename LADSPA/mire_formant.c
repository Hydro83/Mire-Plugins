...
void runMonk(LADSPA_Handle Instance, unsigned long SampleCount) {
    ...
    float vowel_ptr = *p->ports[MONK_VOWEL]; 
    float sharpness = *p->ports[MONK_SHARP]; 
    ...
    for (int i = 0; i < 3; i++) {
        ...
        p->filtersR[i].a0 = p->filtersL[i].a0;
        p->filtersR[i].a1 = p->filtersL[i].a1;
        p->filtersR[i].a2 = p->filtersL[i].a2;
        p->filtersR[i].b1 = p->filtersL[i].b1;
        p->filtersR[i].b2 = p->filtersL[i].b2;
    }
...
void cleanupMonk(LADSPA_Handle Instance) {
    free(Instance);
}
...
const LADSPA_Descriptor * ladspa_descriptor(unsigned long Index) {
    ...
    if (!g_desc) {
        ...
        names[0]="Vowel (A-E-I-O-U)"; 
        names[1]="Sharpness (Q)"; 
        names[2]="Output Gain (dB)";
        ...
        h[0].LowerBound=0; h[0].UpperBound=4; 
        h[0].HintDescriptor=LADSPA_HINT_BOUNDED_BELOW|LADSPA_HINT_BOUNDED_ABOVE|LADSPA_HINT_DEFAULT_0;
        
        h[1].LowerBound=2.0; h[1].UpperBound=40.0; 
        h[1].HintDescriptor=LADSPA_HINT_BOUNDED_BELOW|LADSPA_HINT_BOUNDED_ABOVE|LADSPA_HINT_DEFAULT_LOW;
        
        h[2].LowerBound=-60; h[2].UpperBound=20; 
        h[2].HintDescriptor=LADSPA_HINT_BOUNDED_BELOW|LADSPA_HINT_BOUNDED_ABOVE|LADSPA_HINT_DEFAULT_0;
        
        g_desc->PortRangeHints = h;
        g_desc->instantiate = instantiateMonk; 
        g_desc->connect_port = connectPortMonk; 
        g_desc->run = runMonk; 
        g_desc->cleanup = cleanupMonk;
    }
    return g_desc;
}
